...
Что делает:
• Каждые N минут забегает, берёт список завершённых матчей ESPN за PT-день
• На каждый матч строит пост: счёт (из ESPN, если спорный), рекорды (ESPN), игроки (sports.ru — если уже появились)
• Анти-дубликат: маркеры в каталоге posted/ по eventId с состояниями quick/full
    - quick  → уже опубликован быстрый пост (без таблиц игроков)
    - full   → опубликован (или известны игроки); повторно НИКОГДА не публикуем
    - если был quick и появились игроки → апгрейдим marker в full БЕЗ репоста
• День берётся по PT (Pacific Time), чтобы все матчи дня попадали в одно число.

ENV:
- TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID — куда слать
- TEAM_EMOJI_JSON — {"BOS":"<custom_emoji_id>", ...}
- REPORT_DATE_PT — YYYY-MM-DD (для ретро-тестов)
- DEBUG_NBA=1 — детальные логи

Режимы для тестов (optional):
- SEND_TEST_MESSAGE=1 — отправить ping и выйти
- LIST_EVENTS=1 — вывести завершённые матчи за PT-день и выйти
- ONLY_EVENT_ID=<id> — публиковать только этот матч
- DRY_RUN=1 — не слать в Telegram, только логи
- ALT_CHAT_ID=<id> — слать в альтернативный чат

Замечание:
— Для устранения повторов необходимо, чтобы workflow коммитил каталог posted/ в репозиторий.
"""
...
# Каталог для маркеров (не скрытый, чтобы гарантированно коммитился)
MARKER_DIR = os.getenv("MARKER_DIR", "posted")
...
def make_session():
    ...
    s.mount("http://", ad)
    s.headers.update({
        "User-Agent": "NBA-LiveResultsBot/1.4 (per-game; pt-day; espn-finish; sportsru-players; custom-emoji; fix-scores; full-state)",
...
def pick_report_date_pacific_env() -> date:
    ...
    # до 06:00 PT считаем вчера (чтобы добрать поздние финиши)
...
# Приведение нестандартных аббревиатур к канону
_ABBR_CANON_SWAP = {
...
def load_custom_emoji() -> dict[str, str]:
    if not TEAM_EMOJI_JSON:
        return {}
    try:
...
def emoji_token(abbr: str) -> str:
    # заменится на кастом-эмодзи / дефолт позже, при отправке
...
def _parse_players_table(node) -> list[dict]:
    ...
    for tr in trs:
        ...
        if j.startswith("игрок ") or j.startswith("итого "):
            continue
...
def _extract_total_score_from_page(soup: BeautifulSoup) -> tuple[int, int]:
    ...
    if m:
        ...
        if pairs:
            ...
            return a, b
    # Чтобы не ловить ложные пары типа 23:59 — если нет «заверш», отдаём (0,0)
    return (0, 0)
...
def _espn_record(c: dict) -> str:
    # разные структуры у ESPN, собираем максимально универсально
...
def find_sportsru_match_for_event(sru_games: list[dict], e: dict) -> dict | None:
    ...
    # 2) иначе — тот, где больше игроков (скорее финальная таблица)
    def pcnt(info: dict) -> int:
        Aname = ABBR_TO_RU.get(info["teamA"]["abbr"], info["teamA"]["name"])
        Bname = ABBR_TO_RU.get(info["teamB"]["abbr"], info["teamB"]["name"])
        return len(info["players"].get(Aname, [])) + len(info["players"].get(Bname, []))
    best = max(candidates, key=pcnt)
    return best
...
def _score_suspicious(a: int, b: int) -> bool:
    # отбраковка мусорных/недостоверных значения с сайта
...
def fix_scores_with_espn(info: dict, e: dict):
    ...
    if _score_suspicious(a, b):
        ...
        if canon_abbr(e["home"]["abbr"]) == A["abbr"]:
            A["score"] = int(e["home"]["score"])
            B["score"] = int(e["away"]["score"])
        else:
            A["score"] = int(e["away"]["score"])
            B["score"] = int(e["home"]["score"])

def players_count(info: dict) -> int:
    Aabbr = info["teamA"]["abbr"]
    Babbr = info["teamB"]["abbr"]
    Aname = ABBR_TO_RU.get(Aabbr, info["teamA"]["name"])
    Bname = ABBR_TO_RU.get(Babbr, info["teamB"]["name"])
    return len(info["players"].get(Aname, [])) + len(info["players"].get(Bname, []))

def marker_path(eid: str) -> Path:
    return Path(f"{MARKER_DIR}/event_{eid}.state")

def read_marker_state(eid: str) -> str | None:
    p = marker_path(eid)
    if not p.exists():
        return None
    try:
...
def write_marker_state(eid: str, state: str):
    p = marker_path(eid)
...
def build_block(info: dict) -> str:
    ...
    Aname = ABBR_TO_RU.get(A["abbr"], A["name"])
    Bname = ABBR_TO_RU.get(B["abbr"], B["name"])
    rowsA = info["players"].get(Aname, [])
    rowsB = info["players"].get(Bname, [])
...
def tg_send(text: str):
    ...
    # Заменяем {EMO:(ABBR)} на кастом / дефолтный эмодзи
    entities = []
    out_parts = []
    last = 0
...
def main():
    ...
    # ESPN завершённые матчи PT-дня
    evs = espn_completed_events_for_pt_day(d_pt)
    if ONLY_EVENT_ID:
        evs = [e for e in evs if e["eventId"] == ONLY_EVENT_ID]

    # Парсим sports.ru для PT-дня (в пересечении MSK)
    sru_games = fetch_sports_games_for_pt_day(d_pt)

    posted_now = 0
    for e in evs:
        eid = e["eventId"]
        state = read_marker_state(eid)
        if state == "full":
            log(f"[DBG] SKIP already full {eid}")
            continue

        # матч с players (если есть) или синтетика
        ...
        has_players = players_count(info) > 0

        # Был quick → появились игроки → апгрейд без репоста
        if state == "quick" and has_players:
            write_marker_state(eid, "full")
            log(f"[DBG] UPGRADE quick→full without repost {eid}")
            continue

        # Ещё не публиковали — публикуем
        if state is None:
            text = build_block(info)
            if DRY_RUN:
                log(f"[DRY] post {eid}: {info['teamA']['abbr']}-{info['teamB']['abbr']} "
                    f"{info['teamA']['score']}-{info['teamB']['score']} (players={players_count(info)})")
            else:
                tg_send(text)
                write_marker_state(eid, "full" if has_players else "quick")
                posted_now += 1
                log(f"[DBG] posted event {eid} -> {info['teamA']['score']}-{info['teamB']['score']} "
                    f"state={'full' if has_players else 'quick'}")
            continue

        # state == "quick" и игроков всё ещё нет — пропуск
        log(f"[DBG] SKIP already quick {eid} (no players yet)")
